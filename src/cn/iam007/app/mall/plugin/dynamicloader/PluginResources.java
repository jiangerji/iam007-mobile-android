...
public class PluginResources {
    ...
    public static PluginResources getResource(Class<?> clazz) {
        ...
        return getResource((PluginClassLoader) clazz.getClassLoader());
    }

    static PluginResources getResource(PluginClassLoader mcl) {
        ...
        PluginResources[] rs = null;
        if (mcl.deps != null) {
            rs = new PluginResources[mcl.deps.length];
            for (int i = 0; i < rs.length; i++) {
                PluginResources r = getResource(mcl.deps[i]);
                rs[i] = r;
            }
        }
