...
public class RecommendAdsLayout {
    ...
    private void initView(Context context) {
        mOptions = ImageUtils.getOptionsFadeIn();
        ...
        int height = PlatformUtils.getScreenWidth(mContext) * 480 / 854;
    ...
    private void parseContent(String content) {
        try {
            ...
            ArrayList<ContentInfo> adInfos = ContentInfo.parseJson(object.getJSONArray("d"));
            mAdapter.setAds(adInfos);
            for (int i = 0; i < adInfos.size(); i++) {
                mAdTitles.add(adInfos.get(i).getTitle());
            }
    ...
    private final class AdvAdapter extends PagerAdapter {
        ...
        public void setAds(ArrayList<ContentInfo> adInfos) {
            int size = adInfos.size();
            if (size > 0) {
                mAdvCovers = new ArrayList<String>();

                for (int i = 0; i < size; i++) {
                    mAdvCovers.add(ImageUtils.buildImageUrl(adInfos.get(i)
                            .getThumbnail()));
                }
            }
        }
